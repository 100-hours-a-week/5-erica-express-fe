...
const checkEmailValidation = async email => {
	...
	const isNotCorrectEmail = email && (!emailForm.test(email) || email.length < 5) ? true : false

	if (isNotCorrectEmail) {
		emailText.innerHTML = '* 올바른 이메일 주소 형식을 입력해주세요. (예: example@example.com)'
		return false
	}
...
const checkPasswordValidation = (password, passwordCheck) => {
	...
	if (isNotPasswordRule) {
		passwordText.innerHTML = '* 비밀번호는 대문자, 소문자, 숫자, 특수문자가 들어가야 합니다 (8자 이상 20자 이하)'
		return false
	}
...
signUpButton.addEventListener('click', async () => {
	...
	const isPasswordValid = checkPasswordValidation(passwordValue, passwordCheckValue)
	const isNicknameValid = await checkNicknameValidation(nicknameValue)

	if (!isImageValid || !isEmailValid || !isNicknameValid || !profileImageValue) {
